...
uv_req_t* evm_uv_request_create(size_t request_size, evm_val_t jcallback, size_t extra_data_size) {
    ...
    *EVM_UV_REQUEST_CALLBACK(uv_request) = jcallback;
    return uv_request;
}

void evm_uv_request_destroy(uv_req_t* request) {
    evm_val_free(evm_runtime(), *EVM_UV_REQUEST_CALLBACK(request));
    evm_free(request);
}
...
uv_handle_t* evm_uv_handle_create(size_t handle_size, evm_val_t jobject, size_t extra_data_size) {
  ...
  char* request_memory = evm_malloc(
      aligned_request_size + sizeof(evm_uv_handle_data) + extra_data_size);
  uv_handle_t* uv_handle = (uv_handle_t*)request_memory;
  uv_handle->data = request_memory + aligned_request_size;

  EVM_UV_HANDLE_DATA(uv_handle)->jobject = jobject;
  EVM_UV_HANDLE_DATA(uv_handle)->on_close_cb = NULL;
...
static void uv_handle_close_processor(uv_handle_t* handle) {
  evm_uv_handle_data* handle_data = EVM_UV_HANDLE_DATA(handle);
