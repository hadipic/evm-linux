...
uint32_t              PhysFrameBuffer[76050];
#elif defined (__GNUC__)    /* GNU Compiler */
uint32_t              PhysFrameBuffer[76050] __attribute__ ((aligned (16)));
#else                       /* ARM Compiler */
__align(16) uint32_t  PhysFrameBuffer[76050];
#endif
...
void CopyInVirtualBuffer(uint32_t *pSrc, uint32_t *pDst, uint16_t x, uint16_t y, uint16_t xsize, uint16_t ysize)
{
  ...
  Dma2dHandle.Init.OutputOffset   = 1536 - xsize;
...
static uint8_t LCD_Config(void)
{
  ...
  GfxmmuHandle.Init.Interrupts.Activation             = ENABLE;
...
//  }
  ...
  LayerCfg.ImageWidth      = 1536; /* virtual frame buffer contains 768 pixels per line for 24bpp */
