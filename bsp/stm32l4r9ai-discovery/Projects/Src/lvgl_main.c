...
{
    (void) drv; /*Unused*/
	FRESULT res = f_open(&lvgl_file, fn, FA_READ | FA_OPEN_EXISTING);
...
void lvgl_main()
{
	...
	buf_1 = new_malloc(LV_HOR_RES_MAX * LV_VER_RES_MAX * 2);
    lv_disp_drv_t disp_drv;
    lv_indev_drv_t indev_drv;
    lv_disp_buf_init(&disp_buf, buf_1,NULL, LV_HOR_RES_MAX * LV_VER_RES_MAX);
...
void lodepng_free(void* ptr) {
	return lv_mem_free(ptr);
}

WE_FHANDLE we_file_open(const char *path, uint32_t mode) {
    uint32_t fmode = 0;
...
;
	FIL *file = we_malloc(sizeof(FIL));
	f_open(file, path, fmode);
    return file;
}

uint32_t we_file_close(WE_FHANDLE handle) {
    f_close((FIL*)handle);
    return WE_FS_RES_OK;
}

uint32_t we_file_remove(const char *path) {
    return WE_FS_RES_OK;
}

uint32_t we_file_read(WE_FHANDLE handle, void *buf, uint32_t btr, uint32_t *br) {	
	f_read((FIL*)handle, buf, btr, br);
    return WE_FS_RES_OK; 
}

uint32_t we_file_write(WE_FHANDLE handle, void *buf, uint32_t btw, uint32_t *bw) {
    f_read((FIL*)handle, buf, btw, bw);
    return WE_FS_RES_OK;
}

uint32_t we_file_seek(WE_FHANDLE handle, uint32_t pos, uint32_t mode) {
    f_lseek((FIL*)handle, pos);
    return WE_FS_RES_OK;
}

uint32_t we_file_tell(WE_FHANDLE handle, uint32_t *pos) {
    *pos = (uint32_t)f_tell((FIL*)handle);
    return WE_FS_RES_OK;
}

uint32_t we_file_trunc(WE_FHANDLE handle) {
    
}

uint32_t we_file_size(WE_FHANDLE handle) {
	return f_size((FIL*)handle);
}

uint32_t we_file_rename(const char *oldname, const char *newname) {
    return WE_FS_RES_OK;
}

evm_err_t gui_init_ex(evm_t *e) {
    lvgl_main();
    return ec_ok;
}
